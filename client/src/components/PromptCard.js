...
const DeletePromptModal = ({ prompt, open, setOpen }) => {
    ...
    async function handleDelete(e) {
        try {
            const response = await fetch(`/api/prompts/${prompt._id}`, {
                ...
                body: JSON.stringify({ portalId: prompt.portalId })
            });
    ...
    return (
        <Transition.Root show={open} as={Fragment}>
            <Dialog as="div" className="relative z-20" initialFocus={cancelButtonRef} onClose={closeModal}>
                <Transition.Child
                    as={Fragment}
                        ...
                        <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                            ...
                            <div className='px-4 pb-4 pt-5 sm:p-6 sm:pb-4'>
                                <div className="mt-3 text-center sm:mt-0 sm:text-left">
                                    <Dialog.Title as="h3" className="text-base font-semibold leading-6 text-gray-900">Delete prompt</Dialog.Title>
                                </div>
                                <div className="mt-5">
                                    <p className='text-sm'>
                                        If you delete this prompt, all linked data will be lost.
                                    </p>
                                </div>
                            </div>
...
export default function PromptCard({ prompt })  {
    ...
    return (
        <>
        <DeletePromptModal prompt={prompt} open={open} setOpen={setOpen}/>
        <div className="col-span-1 flex items-center justify-center rounded-lg shadow border border-slate-200 bg-white hover:shadow-md hover:bg-slate-50/75 h-40">
            <div className='grid grid-cols-12 w-full h-full'>
                <a href={`/prompts/${prompt._id}`} className="w-full h-full px-4 py-4 col-span-11">
                    <div className="text-zinc-950 text-4xl font-bold">{prompt.name}</div>
                    <div className='mt-5'>
                        <div className="text-zinc-500 text-md">
                            <span className='font-bold text-zinc-800'>{prompt.views} </span>views
                        </div>
                    </div>
                </a>
