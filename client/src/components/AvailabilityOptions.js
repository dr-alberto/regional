...
export const AvailabilityOptions = ({ data, selected }) => {
    ...
    const handleDeleteOption = (id) => {
        ...
        if (selectedIndex > -1) { // only splice array when item is found
    ...
    return (
        <div>
            <div className='mt-2 flex'>
                    ...
                    <Combobox value={newOptionValue} onChange={setNewOptionValue}>
                        {({ open }) => (
                            <div className='relative'>
                                <Combobox.Input onChange={(event) => setQuery(event.target.value)} className={"border-0 relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6"}/>
                                    ...
                                    <Combobox.Options className={"absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"}>
                                        ...
                                        <Combobox.Option key={index} value={option.name}>
                                            {({ active, selected }) => (
                                                <li
                                                    className={`${
                                                    active ? 'bg-indigo-600 text-white cursor-default select-none py-2 pl-3 pr-9' : 'text-gray-900 relative cursor-default select-none py-2 pl-3 pr-9'
                ...
                <button onClick={handleAddOption} className='flex items-center rounded-md bg-white px-2.5 py-1.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50'>
                    <svg xmlns="http://www.w3.org/2000/svg" height="16" width="14" viewBox="0 0 448 512" className='h-3 w-3 me-2'><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>
                    Add
                </button>
            </div>
            ...
            <ul className="mt-4 overflow-auto max-h-40">
                ...
                <li key={index} className="flex items-center">
                    <div className='my-2 flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 w-full'>
                        <input className="mr-2 block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6 w-full"
                            value={option.name}
                        />
                    </div>
                    <button onClick={() => handleDeleteOption(option.id)} className="hover:text-red-600 mx-2">
